...
public class ContaDAOImpl implements ContaDAO, GenericDAO {
    ...
    public void salvar(Conta conta) {
        try{
            ...
            gerenciadorEntidade.createNativeQuery("INSERT INTO Conta (numero_conta, saldo, tipo, id_cliente, id_usuario) VALUES (?, ?, ?, ?, ?)")
                    ...
                    .setParameter(5, getCurrentUserId()) // Define o quinto parâmetro como o ID do usuario
                    .executeUpdate(); // Executa a consulta de atualização
            logDAO.salvarLog(getCurrentUserId(), "INSERT", "Conta", null, "Conta inserida com sucesso", null, conta);
        }catch (Exception e){
            logDAO.salvarLog(getCurrentUserId(), "ERROR", "Conta", null, "Erro ao tentar inserir a conta: " + e.getMessage(), null, conta);
        }
    }
    ...
    public void atualizar(Conta conta) {
        gerenciadorEntidade.createNativeQuery("UPDATE Conta SET numero_conta = ?, saldo = ?, tipo = ?, id_cliente = ?, id_usuario = ? WHERE id_conta = ?")
                ...
                .setParameter(5, conta.getId_usuario())
                .setParameter(6, conta.getId())
                .executeUpdate();try {
            logDAO.salvarLog(getCurrentUserId(), "UPDATE", "Conta", conta.getId(), "Conta atualizada com sucesso", null, conta);
        }catch (Exception e){
            Map<String, String> dadosAntigos = new HashMap<>();
            dadosAntigos.put("id", conta.getId().toString());
            logDAO.salvarLog(getCurrentUserId(), "ERROR", "Conta", conta.getId(), "Erro ao tentar atualizar a conta: " + e.getMessage(), dadosAntigos, conta);
        }
    }
    ...
    public void deletar(Long id) {
        try{
            ...
            logDAO.salvarLog(getCurrentUserId(), "DELETE", "Conta", id, "Conta deletada com sucesso", null, null);
            // Deleta todas as transferências associadas à conta pelo ID da conta
            transferenciaDAO.deletarTransferenciasPorIdConta(id);
        }catch (Exception e){
            logDAO.salvarLog(getCurrentUserId(), "ERROR", "Conta", id, "Erro ao tentar deletar a conta: " + e.getMessage(), null, null);
        }
    }
    ...
    public void deletarContaPorNumero(Integer numeroConta) {
        ...
        gerenciadorEntidade.createNativeQuery("DELETE FROM Conta WHERE numero_conta = ?")
                .setParameter(1, numeroConta).executeUpdate();
        try {
            logDAO.salvarLog(getCurrentUserId(), "DELETE", "Conta", null, "Conta deletada pelo numero com sucesso", null, null);
            ...
            logDAO.salvarLog(getCurrentUserId(), "ERROR", "Conta", null, "Erro ao tentar deletar a conta pelo numero: " + e.getMessage(), null, null);
        }
    ...
    public void deletarContasPorCpfCliente(String cpf) {
        ...
        gerenciadorEntidade.createNativeQuery("DELETE FROM Conta WHERE id_cliente = (SELECT id_cliente FROM Cliente WHERE cpf = ?)")
                .setParameter(1, cpf).executeUpdate();
        try {
            logDAO.salvarLog(getCurrentUserId(), "DELETE", "Conta", null, "Conta deletada pelo cpf do cliente com sucesso", null, null);
            ...
            logDAO.salvarLog(getCurrentUserId(), "ERROR", "Conta", null, "Erro ao tentar deletar a conta pelo cpf do cliente: " + e.getMessage(), null, null);
        }
    }
    ...
    public void deletarContasPorIdCliente(Long id) {
        ...
        try {
            logDAO.salvarLog(getCurrentUserId(), "DELETE", "Conta", id, "Conta deletada pelo id do cliente com sucesso", null, null);
            ...
            logDAO.salvarLog(getCurrentUserId(), "ERROR", "Conta", id, "Erro ao tentar deletar a conta pelo id do cliente: " + e.getMessage(), null, null);
        }
    }
